        ...
        try:
            ...
            for _, stream_messages in (messages or []):
                for msg_id, fields in stream_messages:
                    ...
                    # Ack even on handler failure: the message is malformed/poisoned
                    # and retrying it forever would wedge the group. Errors are logged.
...
async def start_activity_persister(pool: asyncpg.Pool):
    async def handle(activity: dict[str, Any]):
    ...
    await _consume(settings.STREAM_RESOLVED, "activity-persister", "activity-persister-1", handle)


# ── 2. Review persister: review-queue -> review_queue table ───────────────────

async def start_review_persister(pool: asyncpg.Pool):
    async def handle(item: dict[str, Any]):
        proposed = item.get("proposed_changes")
        if isinstance(proposed, str):
            # extractor already json-encoded it; store as jsonb
            proposed_json = proposed
        else:
            proposed_json = json.dumps(proposed or {})
        await pool.execute(
    ...
    await _consume(settings.STREAM_REVIEW_QUEUE, "review-persister", "review-persister-1", handle)
...
async def start_crm_writer(pool: asyncpg.Pool):
    async def handle(op: dict[str, Any]):
    ...
    await _consume(settings.STREAM_CRM_WRITES, "crm-writer", "crm-writer-1", handle)
