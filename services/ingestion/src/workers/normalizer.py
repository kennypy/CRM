...
# Note: Zoom is deliberately absent — Zoom events never reach raw-signals; the
# gateway publishes them to nexcrm:zoom_events, consumed by the ai-engine
...
async def start_normalizer_workers():
    ...
    async def _process_raw_signal(payload: dict):
        raw_event = RawSignalEvent(**payload)
    ...
    await consume(
        settings.REDIS_URL, settings.STREAM_RAW_SIGNALS, "normalizer", "normalizer-1",
        _process_raw_signal,
        block=1000,
    )
